...
function [outU,outV,outAlpha,outObj,outNumIter] = federated_multi_FCM(sub_data,V,Alpha,c,q,mm,numview,maxIter)
% [~,n] = size(U);
thresh = 10^-7;
gamma = 1;
obj = zeros(maxIter,1);
...
for t = 1:maxIter
    ...
    for subset_index = 1:length(sub_data)
        ...
        part_Alpha{t,subset_index} = Alpha;
    ...
    for p = 1:numview
        dim = size(X{p},1);
        gradient_cur_round{p} = sum(repmat(weight_per_subset, 1, dim*c) .* cur_prototype_gradient{p});
